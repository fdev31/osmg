...
class Avatar {
  ...
  asObject() {
    let r = {};
    for (let k of this.getKeys()) r[k] = this.getValue(k);
    return r;
  }
  asArray() {
  ...
  fromName(name) {
    ...
    let total = Array.from(name)
      .map((c) => c.charCodeAt(0) - 60)
      .reduce((a, b) => a + b);
  ...
  update() {
    ...
    setAttr(
      o.querySelectorAll(`g.hair_front .${d.hairstyle} .tinted`),
      "fill",
      "#" + color[0]
    );
    setAttr(
      o.querySelectorAll(`g.hair_back .${d.hairstyle} .tinted`),
      "fill",
      "#" + color[1]
    );
    setAttr(
